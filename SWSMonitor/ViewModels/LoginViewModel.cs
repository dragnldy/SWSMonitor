...
public class LoginViewModel : ViewModelBase, INotifyPropertyChanged
{
    ...
    /// </summary>
    public string PageTitle => "Sound Water Stewards";
    ...
    public async Task<bool> SignInWithGoogleAsync()
    {
        if (!IsGoodGmailAccount(Email))
        {
            MainWindowViewModel.Current?.ShowStatus("Please enter a valid Gmail address to sign in with Google.", useTimer: true);
            return false;
        }
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(GCID) || string.IsNullOrWhiteSpace(GCS))
            {
                MainWindowViewModel.Current?.ShowStatus("Google OAuth client not configured.", useTimer: true);
                return false;
            }

            var secrets = new ClientSecrets { ClientId = GCID, ClientSecret = GCS };
            ...
            var userInfo = await oauthService.Userinfo.Get().ExecuteAsync().ConfigureAwait(false);
